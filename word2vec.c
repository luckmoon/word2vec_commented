...
void CreateBinaryTree() {
  ...
  for (a = 0; a < vocab_size - 1; a++) {
    ...
    // 第一次循环结束，找到词频最低的两个词，合并一下，min1i=3, min2i=2 (3, 2 是词的idx)
    // {7, 7, 6, 5, 11, 1000000000000000, 1000000000000000, 1000000000000000}
    // 最终循环结束时，
    // (gdb) p *count@7
    // $2 = {7, 7, 6, 5, 11, 14, 25}
    ...
    // (gdb) p *parent_node@7
    // $5 = {5, 5, 4, 4, 6, 6, 0}
    // </s> 的父节点是{7, 7, 6, 5, 11, 14, 25}中idx为5的位置的节点，即14
    // ab 的父节点是{7, 7, 6, 5, 11, 14, 25}中idx为5的位置的节点，即14
    // abc 的父节点是{7, 7, 6, 5, 11, 14, 25}中idx为4的位置的节点，即11
    // 以此类推
    ...
    // (gdb) p *binary@7
    // $4 = {1, 0, 1, 0, 0, 1, 0}
    // 表示 节点 </s> 上的label是1，ab->0, abc->1, abcd->0
    // 节点 11 -> 0, 14->1, 25->0(可能不需要25)
...
void SaveVocab() {
  printf("enter save vocab...\n");
  printf("vocab_size: %lld\n", vocab_size);
  printf("vocab file: %s\n", save_vocab_file);
  long long i;
  FILE *fo = fopen(save_vocab_file, "wb");
  for (i = 0; i < vocab_size; i++) {
    printf("current save word: %s, cnt: %lld\n", vocab[i].word, vocab[i].cn);
    fprintf(fo, "%s %lld\n", vocab[i].word, vocab[i].cn);
  }
  printf("save vocab done.");
  fclose(fo);
}
...
void *TrainModelThread(void *id) {
  ...
  while (1) {
    ...
    if (sentence_length == 0) {
      printf("--------enter if (sentence_length == 0)\n");
      while (1) {
        ...
        if (sentence_length >= MAX_SENTENCE_LENGTH) {
          printf("======if (sentence_length >= MAX_SENTENCE_LENGTH)");
          break;
        }
      }
    ...
    printf("--------exit if (sentence_length == 0)\n");
    ...
    printf("----sentence_position: %lld", sentence_position);
      ...
      if (cw) {
        ...
        if (negative > 0)
          for (d = 0; d < negative + 1; d++) {
            ...
            if (d == 0) {
              // 注意word此时 word = sen[sentence_position];
              target = word;
              label = 1;
        ...
        if (a != window) {
          ...
          if (negative > 0)
            for (d = 0; d < negative + 1; d++) {
              ...
              // g = (label - sigmoid(f))
              // 正样本：(log(sigmoid(x))' = 1 - sigmoid(x)
              // 负样本：(log(sigmoid(-x)))' = 0 - sigmoid(x)
              else g = (label - expTable[(int) ((f + MAX_EXP) * (EXP_TABLE_SIZE / MAX_EXP / 2))]) * alpha;
